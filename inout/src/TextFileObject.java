...
public class TextFileObject {
    ...
    public TextFileObject(String path) throws FileNotFoundException, IOException {
        filepath = path;
        f = new File(filepath);
        String[] filepath_parts = inout.PathMethods.filePathRegExToExtractPartsOfFilePaths(this.filepath);
        filedirectory = filepath_parts[0];
        filename = filepath_parts[1];
        filetype = filepath_parts[2];

        textlines = inout.TextOperations.actualReadFileMethod(path);
    ...
    public void save() throws IOException {
        inout.TextOperations.saveStringArrayToFile(this.filepath, this.textlines);
    }
    ...
    public void saveAs(String path_to_save_to) throws IOException {
        ...
        new File(inout.PathMethods.filePathRegExToExtractPartsOfFilePaths(path_to_save_to)[0]).mkdirs();

        //create the file if not existent, throw Message otherwise
        if (saving.createNewFile() == true) {
            inout.TextOperations.saveStringArrayToFile(path_to_save_to, this.textlines);
        } else {
    ...
    public void saveAsCopyInSameDirectory() throws IOException {
        ...
        inout.TextOperations.saveStringArrayToFile(path, this.textlines);
    }
    ...
    public void move(String path_to_move_to) throws IOException {
        ...
        new File(inout.PathMethods.filePathRegExToExtractPartsOfFilePaths(path_to_move_to)[0]).mkdirs();

        //Create the file if not existent, throw Message otherwise
        if (moving.createNewFile() == true) {
            inout.TextOperations.saveStringArrayToFile(path_to_move_to, this.textlines);
            moved = true;
        } else {
            System.out.println("A file with this name already exists!");
        }
